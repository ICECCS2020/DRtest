...
# We observe that the scipy.optimize.fmin optimizer (using Nelderead method)
...
def clever_score(data_folder='', reduce_op=lambda x: sum(x) / len(x) if len(x) > 0 else 0, user_type='', use_slope=False,
            untargeted=False, num_samples=0, num_images=0,
            shape_reg=0.01, nthreads=0, plot_dir='', method='mle_reg'):
    file_path=data_folder.split('/')
    txt_name='results/'+file_path[-1]+'_'+file_path[-2]+'_'+file_path[-5]+'.txt'
    f=open(txt_name, 'w')
    ...
    file_list = glob.glob(data_folder + '/**/*.mat', recursive = True)
    ...
    for fname in file_list:
        ...
        print(order)
        # keys in mat:
        # ['Li_max', 'pred', 'G1_max', 'g_x0', 'path', 'info', 'G2_max', 'true_label', 'plot_dir, 'L1_max', 'Gi_max', 'L2_max', 'id', 'target_label']
        mat = sio.loadmat(fname)
        print('loading {}'.format(fname))
        
        if order == "1" and use_slope:
        ...
        if num_samples != 0:
            prev_len = len(G1_max)
            G1_max = G1_max[:num_samples]
            G2_max = G2_max[:num_samples]
            Gi_max = Gi_max[:num_samples]
        ...
        img_info = mat['info'][0]
        if user_type != "" and img_info == "user":
            img_info = user_type
        ...
        model = "unknown"
        for path_seg in data_folder.split("/"):
            for n in possible_names:
                if n in path_seg:
                    model = path_seg.replace('_', '-')
                    break
        # model = data_folder.split("/")[1] 
        
        if num_samples == 0: # default, use all G1_max
            figname = 'Fig_'+model+'_'+img_info+'_'+str(true_id)+'_'+str(true_label)+'_'+str(target_label)+'_Nsamp_'+str(len(G1_max));  
        elif num_samples <= len(G1_max) and num_samples > 0:
            figname = 'Fig_'+model+'_'+img_info+'_'+str(true_id)+'_'+str(true_label)+'_'+str(target_label)+'_Nsamp_'+str(num_samples);
        else:
            print('Warning!! Input arg num_samp = {} exceed len(G1_max) in data_process.py'.format(num_samples))
            continue
        ...
        if method == "maxsamp":
            ...
            if order == "1":
                print('estimating L1...')
                Est_G1 = get_lipschitz_estimate(G1_max, "L1", figname, True, shape_reg)
                print('estimating L2...')
                Est_G2 = get_lipschitz_estimate(G2_max, "L2", figname, True, shape_reg)
                print('estimating Li...')
                Est_Gi = get_lipschitz_estimate(Gi_max, "Li", figname, True, shape_reg)
        ...
        if method == "maxsamp":
            if order == "1":
                ...
                f.write('[DEBUG][L1] id = {}, true_label = {}, target_label = {}, info = {}, nsamps = {}, niters = {}, bnd_norm = {}, bnd = {:.5g}'.format(true_id, true_label, target_label, img_info, nsamps, niters, bndnorm_Li, bnd_Li))
                f.write('\n')
            else: # currently only compare L2 bound
                f.write('[DEBUG][L1] id = {}, true_label = {}, target_label = {}, info = {}, nsamps = {}, niters = {}, bnd_norm = {}, bnd = {:.5g}'.format(true_id, true_label, target_label, img_info, nsamps, niters, bndnorm_L2, bnd_L2))
                f.write('\n')
        
        elif method == "mle" or method == "mle_reg":
            if order == "1":
                # estimate Lipschitz constant: Est_G1 is a dictionary containing Lips_est and weibull paras        
                ...
                f.write('[DEBUG][L1] id = {}, true_label = {}, target_label = {}, info = {}, nsamps = {}, niters = {}, bnd_norm = {}, bnd = {:.5g}, ks = {:.5g}, pVal = {:.5g}, shape = {:.5g}, loc = {:.5g}, scale = {:.5g}, g_x0 = {}'.format(true_id, true_label, target_label, img_info, nsamps, niters, bndnorm_Li, bnd_Li, ks_G1, pVal_G1, shape_G1, loc_G1, scale_G1, g_x0))
                f.write('\n')    
            else: # currently only compare L2 bound
                f.write('[DEBUG][L1] id = {}, true_label = {}, target_label = {}, info = {}, nsamps = {}, niters = {}, bnd_norm = {}, bnd = {:.5g}, ks = {:.5g}, pVal = {:.5g}, shape = {:.5g}, loc = {:.5g}, scale = {:.5g}, g_x0 = {}'.format(true_id, true_label, target_label, img_info, nsamps, niters, bndnorm_L2, bnd_L2, ks_G2, pVal_G2, shape_G2, loc_G2, scale_G2, g_x0))
                f.write('\n')
    ...
    if untargeted:
        ...
        for true_id, true_id_bounds in bounds.items():
            ...
            f.write('[STATS][L1] image = {:3d}, n_classes = {:3d}, clever_L1 = {:.5g}, clever_L2 = {:.5g}, clever_Li = {:.5g}'.format(true_id, n_classes, img_clever_L1, img_clever_L2, img_clever_Li))
            f.write('\n')
        ...
        f.write('[STATS][L0] info = {}, {}_clever_L1 = {:.5g}, {}_clever_L2 = {:.5g}, {}_clever_Li = {:.5g}'.format(info, info, clever_L1, info, clever_L2, info, clever_Li))
        f.write('\n')
    else:
        ...
        for info, info_bounds in bounds.items():
            ...
            if order == "1":
                ...
                f.write('[STATS][L0] info = {}, {}_clever_L2 = {:.5g}'.format(info, info, clever_L2))
                f.write('\n')
...
def main(argv=None):
    clever_score(data_folder='lipschitz_mat/lenet5_test/mnist_lenet5', untargeted=True)
    
if __name__ == '__main__':
    flags.DEFINE_string('dataset', 'mnist', 'The target datasets.')
